...
def add_stonks(x, y):
    # This function will check if the inputted item is already present in the dictionary keys. If the item is not
    # present, then the function will assign the inputted quantity as the item's value. However, If the item is
    # already present, it will add the quantity to the pre-existing value.
...
def buy_stonks(x, y):
    # This function will check if the inputted item is already present in the dictionary keys. If it isn't then it
    # will let the user know that the stock they want to purchase isn't available.
    if x in stocks.keys():
